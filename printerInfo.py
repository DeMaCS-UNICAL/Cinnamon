...
class PrinterInfo (threading.Thread):
    ...
    HEADER_CLIENT = [' STATION             ', ' AUTH',' DEAUTH ', ' PWR ',' HAND_SUCC ',' HAND_FAIL ',' CORRUPT ',' CORR%',' DATA  ',' RTS ',' CTS ',' ACK',' BEACON  ', ' PROBE_REQ  ', ' PROBE_RESP  ', ' TOT_PACK', ' OTHER']
    
    HEADER_AP = [' ESSID               ',' BSSID             ',' AUTH ',' DEAUTH ',' PWR ',' HAND_SUCC ',' HAND_FAIL ',' CORRUPT ',' CORR% ',' DATA ',' RTS ',' CTS ',' ACK ',' BEACON ', ' PROBE_REQ ', ' PROBE_RESP ', ' TOT_PACK', 'OTHER']
    
    HEADER_INFO = ['ESSID              ','BSSID            ','STATION          ', 'AUTH','DEAUTH', 'PWR','HAND_SUCC','HAND_FAIL','CORRUPT','CORR %','DATA','RTS','CTS','ACK','BEACON', 'PROBE_REQ', 'PROBE_RESP', 'TOT']
                    ...
                    if self.indexTable == 0:
                        ...
                        if self.indexCursorClient < len(self.infoClient) -1:
                            ...
                            if not self.pressedInfo:
                                ...
                                if self.indexCursorClient > PrinterInfo.HEIGHT_TABLE_CLIENT:
                                    if self.mypad_pos_client > 15:
                                        self.mypad_pos_client = self.mypad_pos_client + 1
                                ...
                                if self.indexCursorClient > PrinterInfo.HEIGHT_TABLE_CLIENT - self.contInfoClient - 8:
                        ...
                        if self.pressedInfo:
                            for elem in self.info:
                                if self.tupl and (elem[0], self.tupl[0][0]) in self.info:
                                    ...
                                    if i[0] == None or i[0] == "":
                                        bssid = "-"
                                    tup = tuple([[i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8], perc, i[10], i[11], i[12], i[13], i[14], i[15], i[16], i[17]]])
    ...
    def addInfo(self,i):
        ...
        if (i[1],i[2]) not in self.info:
            ...
            if i[0] == "-" and (i[1],i[2]) in self.essid and self.essid[(i[1],i[2])] != "":
                j = tuple([self.essid[(i[1],i[2])], i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8], i[9], i[10], i[11], i[12], i[13], i[14], i[15], i[16], i[17], i[18]])
                self.info[(i[1],i[2])] = j
            else:
                self.info[(i[1],i[2])] = i
    ...
    def printInformation(self):
        ...
        for i in self.sort_tableClient(self.indexOrdClient):
            ...
            tup = tuple([[a[0][0], a[1][0], a[2][0], a[3][0], a[4][0], a[5][0], a[6][0], perc, a[8][0], a[9][0], a[10][0], a[11][0], a[12][0], a[13][0], a[14][0], a[15][0], a[16][0]]])
        ...
        for i in self.sort_tableAP(self.indexOrdAP):
            ...
            tup = tuple([[a[0][0], a[1][0], a[2][0], a[3][0], a[4][0], a[5][0], a[6][0], a[7][0], perc, a[9][0], a[10][0], a[11][0], a[12][0], a[13][0], a[14][0], a[15][0], a[16][0], a[17][0]]])
    ...
    def createTable(self, header_client, header_ap):
        ...
        self.tableAP.set_cols_align(["l", "r", "c", "c","c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c"])
        self.tableAP.set_cols_valign(["t", "b", "m", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b", "b"])
        
        self.tableAP.add_rows([[header_ap[0], header_ap[1], header_ap[2], header_ap[3], header_ap[4], header_ap[5], header_ap[6], header_ap[7], header_ap[8], header_ap[9], header_ap[10], header_ap[11], header_ap[12], header_ap[13],  header_ap[14], header_ap[15], header_ap[16], header_ap[17]]])
        ...
        self.table.add_rows([[header_client[0], header_client[1], header_client[2], header_client[3], header_client[4], header_client[5], header_client[6], header_client[7], header_client[8], header_client[9], header_client[10], header_client[11], header_client[12],  header_client[13], header_client[14], header_client[15], header_client[16]]])
